...
export default function DHScheduleCreatePage() {
  ...
  const checkSectionConflict = useCallback((idx: number, sec: SectionData) => {
    ...
    conflictTimers.current[idx] = setTimeout(async () => {
      try {
        ...
        // Ignore transient conflict check failures to avoid blocking form input.
      } finally {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        const [dept, ayRes, roomRes] = await Promise.all([
        ...
        const current = ayRes.data.find((a) => a.isCurrent) || null
        setActiveAy(current)
        if (current) {
        ...
        // Keep page usable even if one lookup fails.
      } finally {
        setLoading(false)
      }
    }
  ...
  useEffect(() => {
    dhLookupsApi
      .subjects({ semester: semester || undefined })
      .then((res) => {
        setSubjects(res.data)
        setSubjectId((prev) => (prev && !res.data.some((s) => String(s.id) === prev) ? '' : prev))
      })
      .catch(() => setSubjects([]))
  }, [semester])
  ...
  useEffect(() => {
    ...
    dhSchedulesApi
      .list({ limit: 200, academic_year_id: academicYearId, semester })
      .then((res) => {
        ...
        setExistingSections(matching.map((s) => (s.section || '').toUpperCase()))
      })
      .catch(() => setExistingSections([]))
  }, [subjectId, academicYearId, semester])
  ...
  function validate(): boolean {
    ...
    if (!academicYearId) nextErrors.push('Academic Year is required.')
    if (!semester) nextErrors.push('Semester is required.')
    if (sections.length === 0) nextErrors.push('At least one section is required.')

    sections.forEach((sec, i) => {
      const resolved = resolveSectionSchedule(sec)
      const timeRangeError = getTimeRangeError(resolved.startTime, resolved.endTime)
      if (!sec.section.trim()) nextErrors.push(`Section ${i + 1}: Section name is required.`)
      if (!sec.roomId) nextErrors.push(`Section ${i + 1}: Room is required.`)
      if (!resolved.dayPattern) nextErrors.push(`Section ${i + 1}: Day pattern is required.`)
      if (!resolved.startTime || !resolved.endTime) nextErrors.push(`Section ${i + 1}: Time slot is required.`)
      if (timeRangeError) nextErrors.push(`Section ${i + 1}: ${timeRangeError}`)
    })
  ...
  async function handleSubmit(ev: FormEvent) {
    ...
    try {
      ...
      for (const sec of sections) {
        try {
          const resolved = resolveSectionSchedule(sec)
          const response = await dhSchedulesApi.create({
          ...
          errorMsgs.push(`Section ${sec.section}: ${err?.response?.data?.message || 'Failed to create.'}`)
        }
      }
      ...
      if (errorMsgs.length > 0) {
        ...
        router.push('/department-head/schedules')
      }
    } finally {
      setSaving(false)
    }
    ...
    <div className="mx-auto w-full max-w-6xl space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Link href="/department-head/schedules" className="p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-800">
        ...
        <Link href="/department-head/schedules">
          <Button size="sm" variant="secondary">Back to Schedules</Button>
        </Link>
      </div>
                                    ...
                                    <>
                                      ...
                                      <div className="flex items-center gap-1.5">
                                        {c.type === 'room_time_conflict' && (
                                          <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-red-100 text-red-700 font-semibold">🏫 Room Conflict</span>
                                        )}
                                        {c.type === 'faculty_conflict' && (
                                          <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-orange-100 text-orange-700 font-semibold">👤 Faculty Conflict</span>
                                        )}
                                        {c.type === 'section_conflict' && (
                                          <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-purple-100 text-purple-700 font-semibold">📋 Section Conflict</span>
                                        )}
                                      </div>
            ...
            <Card>
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-3">
                  <Link href="/department-head/schedules"><Button size="sm" variant="secondary">Cancel</Button></Link>
                  <Button
                    size="sm"
                    type="submit"
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200 dark:border-amber-800 p-5">
              ...
              <ul className="space-y-2 text-xs text-amber-900 dark:text-amber-200">
                ...
                <li className="flex items-start gap-2"><span className="text-amber-600">&bull;</span>Check that times do not overlap with other classes</li>
                <li className="flex items-start gap-2"><span className="text-amber-600">&bull;</span>Standard class duration is 1 to 3 hours</li>
