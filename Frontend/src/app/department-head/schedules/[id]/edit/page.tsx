...
import { dhSchedulesApi, dhLookupsApi } from '@/lib/department-head-api'
import { DAY_PATTERNS, TIME_SLOTS, SCHEDULE_STATUSES } from '@/lib/constants'
import { formatTime } from '@/lib/utils'
import type { ScheduleInput, ConflictCheckResult, Subject, Room } from '@/types'
...
export default function DHScheduleEditPage() {
  ...
  useEffect(() => {
    Promise.all([
      dhSchedulesApi.get(Number(id)),
      dhLookupsApi.subjects().then((r) => r.data),
      dhLookupsApi.rooms().then((r) => r.data),
    ])
      .then(([sched, sub, rm]) => {
        const initialForm: ScheduleInput = {
          semester: sched.semester,
          dayPattern: sched.dayPattern || 'M-W-F',
          startTime: sched.startTime,
          endTime: sched.endTime,
          section: sched.section || '',
          enrolledStudents: sched.enrolledStudents,
          notes: sched.notes || '',
          academicYearId: sched.academicYear.id,
        ...
        setAcademicYearLabel(sched.academicYear.year)
      ...
      .catch(() => setServerError('Failed to load schedule data.'))
  ...
  async function handleSubmit(ev: FormEvent) {
    ...
    try {
      const resolved = resolveScheduleInput(form, useCustomFormat, customDays, customStartTime, customEndTime)
      const response = await dhSchedulesApi.update(Number(id), {
        ...form,
        dayPattern: resolved.dayPattern,
        startTime: resolved.startTime,
        endTime: resolved.endTime,
      ...
      router.push(`/department-head/schedules/${id}`)
    } catch (err: any) {
      setServerError(err?.response?.data?.message || 'Failed to update schedule.')
    } finally {
      setSaving(false)
    }
  }
  ...
  const statusOptions = SCHEDULE_STATUSES.map((s) => ({ value: s, label: s.charAt(0).toUpperCase() + s.slice(1) }))
  ...
  const roomOptions = [{ value: '', label: '-- Select Room --' }, ...rooms.map((r) => ({ value: String(r.id), label: `${r.code}${r.name ? ` - ${r.name}` : ''}` }))]

  return (
    <div className="mx-auto w-full max-w-6xl space-y-6">
      <div className="flex items-center gap-3">
        <Link href={`/department-head/schedules/${id}`} className="p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-800">
          <ArrowLeft className="h-5 w-5 text-gray-500 dark:text-gray-400" />
        </Link>
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Edit Schedule</h1>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Update schedule details with real-time conflict detection</p>
        </div>
      </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {!useCustomFormat && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <Select
                    ...
                    onChange={(e) => set('dayPattern', e.target.value)}
                    options={dayOptions}
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <Input
                      ...
                      type="time"
                      value={customEndTime}
                      onChange={(e) => updateCustomTime('customEndTime', e.target.value)}
                    />
                  </div>
                  {timeRangeError && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <Input
                  ...
                  type="number"
                  value={String(form.enrolledStudents ?? '')}
                  onChange={(e) => set('enrolledStudents', e.target.value ? Number(e.target.value) : undefined)}
                />
                <Select label="Status" value={form.status || 'draft'} onChange={(e) => set('status', e.target.value)} options={statusOptions} />
              </div>

              <Textarea label="Notes" value={form.notes || ''} onChange={(e) => set('notes', e.target.value)} rows={2} />

              <div className="flex justify-end gap-3 pt-4">
                <Link href={`/department-head/schedules/${id}`}><Button size="sm" variant="secondary">Cancel</Button></Link>
                <Button size="sm" type="submit" icon={<Save className="h-4 w-4" />} disabled={saving || (conflicts?.hasConflict ?? false)}>
