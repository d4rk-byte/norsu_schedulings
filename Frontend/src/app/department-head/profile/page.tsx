...
import { dhProfileApi, type DhProfileData, type DhProfileUpdateInput } from '@/lib/department-head-api'

export default function DeptHeadProfilePage() {
  const [profile, setProfile] = useState<DhProfileData | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState('')
  const [success, setSuccess] = useState('')
  const [saving, setSaving] = useState(false)
  const [editing, setEditing] = useState(false)
  const [form, setForm] = useState<DhProfileUpdateInput>({})
  ...
  const handleSave = async () => {
    ...
    setSuccess('')
    try {
      await dhProfileApi.update(form)
      setSuccess('Profile updated successfully.')
      setEditing(false)
      const updated = await dhProfileApi.get()
      setProfile(updated)
    } catch {
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4">
          ...
          <div className="text-center sm:text-left">
            ...
            <div className="flex flex-wrap justify-center sm:justify-start gap-2 mt-2">
              <Badge variant="primary">Department Head</Badge>
