...
export default function EditCollegePage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setForm({ code: data.code, name: data.name, description: data.description || '', dean: data.dean || '' })
      } catch {
        setServerError('Failed to load college.')
      } finally {
        setLoading(false)
      }
    }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setServerError(null)
    try {
      await collegesApi.update(Number(id), form)
  ...
  return (
    <div className="mx-auto w-full max-w-5xl space-y-6">
      <div className="flex items-center gap-4">
        <Link href={`/admin/colleges/${id}`} className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700">
          <ArrowLeft className="h-5 w-5 text-gray-500" />
        </Link>
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Edit College</h1>
          <p className="mt-1 text-sm text-gray-500">Update college information.</p>
        </div>
      </div>
      ...
      <Card className="max-w-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t">
            <Link href={`/admin/colleges/${id}`}><Button variant="secondary" type="button">Cancel</Button></Link>
