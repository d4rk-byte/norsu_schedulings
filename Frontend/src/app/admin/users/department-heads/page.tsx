...
export default function DepartmentHeadsPage() {
  ...
  const list = useCrudList<User>((p) => usersApi.list({ ...p, role: ROLES.DEPARTMENT_HEAD }))
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-3xl font-bold text-gray-900 dark:text-white">Department Heads</h1><p className="mt-1 text-sm text-gray-500">Department head users in the system.</p></div>
      <Card>
        <div className="mb-4 space-y-3">
          <div className="flex flex-wrap items-center gap-3">
            <SearchBar value={list.search} onChange={list.setSearch} placeholder="Search department heads..." className="max-w-sm" />
        ...
        <DataTable columns={columns} data={list.data} keyExtractor={(u) => u.id} loading={list.loading} sort={list.sort} onSort={list.setSort} onRowClick={(u) => router.push(`/admin/users/${u.id}`)} emptyTitle="No department heads found" />
