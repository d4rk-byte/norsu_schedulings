...
export default function CreateRoomPage() {
  ...
  useEffect(() => {
    Promise.all([departmentsApi.list({ limit: 200 }), departmentGroupsApi.list({ limit: 100 })])
      .then(([d, g]) => { setDepartments(d.data); setGroups(g.data) })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try { await roomsApi.create(form); router.push('/admin/rooms') } catch (err: unknown) { setServerError(err instanceof Error ? err.message : 'Failed to create room') } finally { setSaving(false) }
  }
  ...
  return (
    <div className="mx-auto w-full max-w-5xl space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div><h1 className="text-3xl font-bold text-gray-900 dark:text-white">Create Room</h1><p className="mt-1 text-sm text-gray-500">Add a new room to the system.</p></div>
      </div>
      ...
      <Card className="max-w-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Input label="Code" value={form.code} onChange={(e) => update('code', e.target.value)} error={errors.code} placeholder="e.g. RM101" required />
            <Input label="Name" value={form.name || ''} onChange={(e) => update('name', e.target.value)} placeholder="Room name (optional)" />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <Input label="Floor" value={form.floor || ''} onChange={(e) => update('floor', e.target.value)} placeholder="e.g. 2nd" />
          </div>
          <Input label="Building" value={form.building || ''} onChange={(e) => update('building', e.target.value)} placeholder="Building name" />
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t">
            <Link href="/admin/rooms"><Button variant="secondary" type="button">Cancel</Button></Link>
