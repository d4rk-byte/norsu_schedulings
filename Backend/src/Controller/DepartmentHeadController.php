...
class DepartmentHeadController extends AbstractController
{
    ...
    #[Route('/schedule-change-requests', name: 'schedule_change_requests')]
    public function scheduleChangeRequests(): Response
    {
        return $this->render('department_head/schedule_change_requests/index.html.twig', array_merge($this->getBaseTemplateData(), [
            'page_title' => 'Schedule Change Requests',
        ]));
    }
    ...
    public function historyReports(Request $request): Response
    {
        ...
        // Get department group IDs (for rooms/faculty/subjects which are shared resources)
        $departmentIds = $this->getDepartmentGroupIds($department);
    ...
    private function buildHistorySubjectsData(array $departmentIds, array $schedules): array
    {
        ...
        // History should include subjects that appear in historical schedules,
        // even if the subject record is currently soft-deleted.
        foreach ($schedules as $schedule) {
            $subject = $schedule->getSubject();
            if (!$subject || !$subject->getDepartment()) {
                continue;
            }

            if (!in_array($subject->getDepartment()->getId(), $departmentIds, true)) {
                continue;
            }
        ...
        // Normalize to indexed array for deterministic iteration in Twig output.
        ...
        if ($request->isMethod('POST')) {
            try {
                ...
                foreach ($sectionsData as $index => $sectionData) {
                    ...
                    $dayPattern = $this->conflictDetector->normalizeDayPattern($sectionData['day_pattern'] ?? '');
                    $startTime = $this->conflictDetector->normalizeTimeInput($sectionData['start_time'] ?? '');
                    $endTime = $this->conflictDetector->normalizeTimeInput($sectionData['end_time'] ?? '');
    ...
    public function checkConflicts(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $dayPattern = $this->conflictDetector->normalizeDayPattern($data['day_pattern'] ?? null);
            $startTime = $this->conflictDetector->normalizeTimeInput($data['start_time'] ?? null);
            $endTime = $this->conflictDetector->normalizeTimeInput($data['end_time'] ?? null);
    ...
    public function checkRoomConflicts(Request $request): JsonResponse
    {
        try {
            ...
            $dayPattern = $this->conflictDetector->normalizeDayPattern($data['dayPattern'] ?? null);
            $startTime = $this->conflictDetector->normalizeTimeInput($data['startTime'] ?? null);
            $endTime = $this->conflictDetector->normalizeTimeInput($data['endTime'] ?? null);
            $semester = $data['semester'] ?? null;
    ...
    public function editSchedule(Request $request, Schedule $schedule): Response
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            $dayPattern = $this->conflictDetector->normalizeDayPattern($request->request->get('day_pattern'));
            $startTime = $this->conflictDetector->normalizeTimeInput($request->request->get('start_time'));
            $endTime = $this->conflictDetector->normalizeTimeInput($request->request->get('end_time'));
