...
class ScheduleController extends AbstractController
{
        ...
        if ($request->isMethod('POST')) {
            try {
                ...
                foreach ($sectionsData as $index => $sectionData) {
                    ...
                    $dayPattern = $conflictDetector->normalizeDayPattern($sectionData['day_pattern'] ?? '');
                    $startTime = $conflictDetector->normalizeTimeInput($sectionData['start_time'] ?? '');
                    $endTime = $conflictDetector->normalizeTimeInput($sectionData['end_time'] ?? '');
    ...
    public function edit(Request $request, Schedule $schedule, \App\Service\ScheduleConflictDetector $conflictDetector): Response
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            $dayPattern = $conflictDetector->normalizeDayPattern($request->request->get('day_pattern'));
            $startTime = $conflictDetector->normalizeTimeInput($request->request->get('start_time'));
            $endTime = $conflictDetector->normalizeTimeInput($request->request->get('end_time'));
    ...
    public function checkConflict(Request $request, \App\Service\ScheduleConflictDetector $conflictDetector): JsonResponse
    {
        ...
        try {
            ...
            $normalizedDayPattern = $conflictDetector->normalizeDayPattern($data['day_pattern'] ?? null);
            $normalizedStartTime = $conflictDetector->normalizeTimeInput($data['start_time'] ?? null);
            $normalizedEndTime = $conflictDetector->normalizeTimeInput($data['end_time'] ?? null);

            if (!$normalizedDayPattern || !$normalizedStartTime || !$normalizedEndTime) {
                return $this->json([
                    'has_conflicts' => false,
                    'conflicts' => ['Invalid day pattern or time format.'],
                    'warnings' => [],
                    'debug' => 'Normalization failed for day/time'
                ]);
            }

            $schedule->setDayPattern($normalizedDayPattern);
            $schedule->setStatus('active');
            
            // Parse times
            $schedule->setStartTime(new \DateTime($normalizedStartTime));
            $schedule->setEndTime(new \DateTime($normalizedEndTime));
    ...
    public function checkRoomConflicts(Request $request, \App\Service\ScheduleConflictDetector $conflictDetector): JsonResponse
    {
        try {
            ...
            $dayPattern = $conflictDetector->normalizeDayPattern($data['dayPattern'] ?? null);
            $startTime = $conflictDetector->normalizeTimeInput($data['startTime'] ?? null);
            $endTime = $conflictDetector->normalizeTimeInput($data['endTime'] ?? null);
