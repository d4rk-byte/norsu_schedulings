...
class ActivityLogService
{
    ...
    public function logUserActivity(string $action, User $targetUser, ?array $metadata = null): ActivityLog
    {
        $descriptions = [
            ...
            'user.password_changed' => "Password changed for {$targetUser->getFirstName()} {$targetUser->getLastName()}",
            'faculty.created' => "New faculty {$targetUser->getFirstName()} {$targetUser->getLastName()} was created",
            'faculty.updated' => "Faculty {$targetUser->getFirstName()} {$targetUser->getLastName()} was updated",
        ];

        $description = $descriptions[$action] ?? "User activity: {$action}";
        $isSessionAction = $action === 'user.login' || $action === 'user.logout';

        // Guard against duplicated auth events firing within a tiny time window.
        if ($isSessionAction) {
            $ipAddress = $this->requestStack->getCurrentRequest()?->getClientIp();
            $recentDuplicate = $this->activityLogRepository->findRecentDuplicateForUser(
                $targetUser,
                $action,
                5,
                $ipAddress
            ...
            if ($recentDuplicate instanceof ActivityLog) {
        ...
        return $this->log(
            ...
            $targetUser->getId(),
            $metadata,
            $isSessionAction ? $targetUser : null
        );
    }
