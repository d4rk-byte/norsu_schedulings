...
class SystemSettingsService
{
    ...
    private const SETTING_AUTO_ACTIVATE_NEW_USERS = 'auto_activate_new_users';
    private const DEFAULT_AUTO_ACTIVATE_NEW_USERS = false;
    ...
    /**
     * Whether newly registered users should be auto-activated.
     */
    public function isAutoActivateNewUsersEnabled(): bool
    {
        try {
            $rawValue = $this->entityManager->getConnection()->fetchOne(
                'SELECT setting_value FROM app_settings WHERE setting_key = :settingKey LIMIT 1',
                ['settingKey' => self::SETTING_AUTO_ACTIVATE_NEW_USERS]
            );
        } catch (\Throwable $e) {
            $this->logger?->warning('Failed to read auto-activation setting. Falling back to default.', [
                'exception' => $e,
            ]);

            return self::DEFAULT_AUTO_ACTIVATE_NEW_USERS;
        }
        ...
        return $parsed ?? self::DEFAULT_AUTO_ACTIVATE_NEW_USERS;
    }

    /**
     * Enable or disable automatic activation for newly registered users.
     */
    public function setAutoActivateNewUsersEnabled(bool $enabled): void
    {
        $this->entityManager->getConnection()->executeStatement(
            'INSERT INTO app_settings (setting_key, setting_value, created_at, updated_at)
             VALUES (:settingKey, :settingValue, NOW(), NOW())
